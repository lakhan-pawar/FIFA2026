  ...
  return (
    ...
    >
      ...
      <div className={`px-4 py-3 border-b flex justify-between items-center ${
        isFavGroup ? 'border-[var(--accent)]/20 bg-[var(--accent)]/5' :
        isCanadaGroup ? 'border-[#cc0000]/20 bg-[#cc0000]/5' : 'border-[var(--border)] bg-[var(--bg-2)]'
      }`}>
        <div className="flex items-center gap-2">
          {isFavGroup ? <Star className="w-3.5 h-3.5 text-[var(--accent)] fill-[var(--accent)]" /> :
           isCanadaGroup ? <span className="text-base">🍁</span> : null}
          <h3 className={`font-display text-lg ${isFavGroup ? 'text-[var(--accent)]' : isCanadaGroup ? 'text-[#ff4d6d]' : ''}`}>
            Group {groupLetter}
          </h3>
          {isFavGroup && (
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] font-mono text-[var(--muted)]">{filled}/{TOTAL_PER_GROUP}</span>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 hover:bg-[var(--bg-2)] transition-colors group cursor-pointer ${
                isUserFavorite ? 'bg-[var(--accent)]/5' : isCanada ? 'bg-[#cc0000]/5' : ''
              }`}
            >
              <span className="text-xs font-bold text-[var(--muted)] w-4 italic">{idx + 1}</span>
              ...
              <span className={`flex-1 font-semibold text-sm transition-colors ${
                ...
                {isUserFavorite && <span className="ml-2 text-[10px] opacity-70">(Selected)</span>}
              </span>
              <div className="flex gap-3 text-[10px] font-mono text-[var(--muted)]">
                <span>P:0</span><span>W:0</span><span>L:0</span>
                <span className="font-bold text-[var(--text-2)]">0pts</span>
              </div>
              <span className="text-[var(--border)] group-hover:text-[var(--accent)] text-sm transition-colors">›</span>
            </Link>
          );
        })}

        {Array.from({ length: TOTAL_PER_GROUP - filled }).map((_, i) => (
          <div key={`empty-${i}`} className="flex items-center gap-3 px-4 py-3 opacity-25">
            <span className="text-xs font-bold text-[var(--muted)] w-4">{filled + i + 1}</span>
            <div className="w-7 h-7 rounded-full bg-[var(--bg-2)] border border-[var(--border)] flex items-center justify-center text-xs font-bold text-[var(--muted)]">?</div>
            <span className="flex-1 text-xs italic text-[var(--muted)]">Qualifying TBD</span>
          </div>
        ))}
      </div>
    </div>
...
export default function StandingsPage() {
  ...
  const favGroupLetter = favTeam ? MAJOR_TEAMS.find(t => t.name === favTeam.name)?.group : null;
  ...
  const filteredGroups = GROUPS.filter(g => {
    ...
    const teams = getTeamsInGroup(g);
    return teams.some(t => t.name.toLowerCase().includes(search.toLowerCase()) || `group ${g}`.toLowerCase().includes(search.toLowerCase()));
  });
    ...
    <div className="w-full max-w-[1100px] mx-auto px-4 py-8 pb-24">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--muted)]">
          The road to glory. 12 groups, 4 teams each. Top 2 + 8 best third-placed teams advance.
        </p>
      </div>

      {/* ── SPOTLIGHT ── */}
      <div className={`mb-8 p-5 rounded-2xl bg-gradient-to-br border relative overflow-hidden ${
        isFavSpotlight 
          ? 'from-[var(--accent)]/15 via-[var(--card)] to-[var(--bg-2)] border-[var(--accent)]/30'
          : 'from-[#cc0000]/15 via-[var(--card)] to-[var(--bg-2)] border-[#cc0000]/30'
      }`}>
        <div className={`absolute -top-10 -right-10 w-40 h-40 blur-3xl rounded-full pointer-events-none ${
          isFavSpotlight ? 'bg-[var(--accent)]/8' : 'bg-[#cc0000]/8'
        }`} />
        <div className="relative z-10 flex items-start gap-4">
          <div className={`w-14 h-14 rounded-xl border flex items-center justify-center text-3xl shrink-0 ${
            isFavSpotlight 
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h2 className="font-display text-xl text-[var(--text)]">
                {isFavSpotlight ? `${favTeam?.name}'s Journey` : "Canada's Path to Glory"}
              </h2>
            </div>
            <p className="text-xs text-[var(--muted)] mb-3">
              ...
              <strong className={isFavSpotlight ? 'text-[var(--accent)]' : 'text-[#ff4d6d]'}>Group {spotlightGroup}</strong>. 
              {isFavSpotlight && ` Support them as they chase the dream!`}
              {!isFavSpotlight && ` Their 2nd World Cup appearance — and first on home soil.`}
            </p>
              ...
              {getTeamsInGroup(spotlightGroup).map(t => (
                <Link
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--bg-2)] border border-[var(--border)] transition-colors text-xs font-semibold ${
                    t.name === favTeam?.name ? 'border-[var(--accent)]/40 text-[var(--accent)]' :
                    t.name === 'Canada' ? 'border-[#ff4d6d]/40 text-[#ff4d6d]' : 'hover:border-[var(--accent)]/20 shadow-sm'
                  }`}
                >
                  <img src={`https://flagcdn.com/w40/${t.flagCode}.png`} className="w-4 h-4 rounded-full object-cover" alt={t.name} />
                  {t.name === 'Canada' ? '🍁 Canada' : t.name}
                  {t.name === favTeam?.name && <span className="text-[8px] opacity-70 ml-1">★</span>}
                </Link>
              ))}
              {Array.from({ length: TOTAL_PER_GROUP - getTeamsInGroup(spotlightGroup).length }).map((_, i) => (
                <span key={i} className="px-3 py-1.5 rounded-full bg-[var(--bg-2)] border border-[var(--border)] text-xs text-[var(--muted)] italic">TBD</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex items-start gap-4 p-4 rounded-2xl bg-[var(--accent-3)]/8 border border-[var(--accent-3)]/20 mb-8">
        ...
        <div>
          <h4 className="text-sm font-bold text-[var(--text)] mb-0.5">Expanded 48-Team Format</h4>
          <p className="text-xs text-[var(--muted)] leading-relaxed">
            FIFA WC 2026 features <strong className="text-[var(--text)]">12 groups of 4</strong>. Top 2 from each + 8 best 3rd-placed teams advance to the <strong className="text-[var(--text)]">Round of 32</strong>.
          </p>
        </div>
      </div>
