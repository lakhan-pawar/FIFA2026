...
const VENUES: VenueData[] = TORONTO_VENUES.map(v => ({ ...v, amenities: v.amenities }));

// Simulated vibe data (deterministic by id)
const VIBE_DATA: Record<string, { rush: 'Chill' | 'Packed' | 'Legendary'; score: number }> = {
...
const RUSH_CONFIG = {
  Chill:     { color: '#00e5a0', bg: 'rgba(0,229,160,0.1)', border: 'rgba(0,229,160,0.25)', pulse: false },
  Packed:    { color: '#ffd700', bg: 'rgba(255,215,0,0.1)', border: 'rgba(255,215,0,0.25)', pulse: true },
  Legendary: { color: '#ff4d6d', bg: 'rgba(255,77,109,0.1)', border: 'rgba(255,77,109,0.3)', pulse: true },
};
...
function VenueCard({ venue, distance }: { venue: VenueData; distance?: number }) {
    ...
    <div className="group p-5 rounded-2xl bg-[var(--card)] border border-[var(--border)] hover:border-[var(--border-hover)] transition-all">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[var(--muted)] truncate">📍 {venue.address}</p>
        </div>
        <div className="flex flex-col items-end gap-1 shrink-0">
          {/* Crowd-rush badge */}
          <div
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border"
            style={{ color: rush.color, background: rush.bg, borderColor: rush.border }}
          >
            {vibe.rush === 'Legendary' && (
              <span className="w-1.5 h-1.5 rounded-full animate-pulse inline-block" style={{ background: rush.color }} />
            )}
            {vibe.rush === 'Packed' && (
              <span className="w-1.5 h-1.5 rounded-full animate-pulse inline-block" style={{ background: rush.color }} />
            )}
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="flex gap-0.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <span key={i} className={`text-sm ${i < vibe.score ? 'opacity-100' : 'opacity-20'}`}>⚽</span>
          ))}
        </div>
        <span className="text-[10px] text-[var(--muted)]">Vibe Score</span>
        <span className="text-[10px] text-[var(--muted)] ml-auto">Max fans: {venue.capacity}</span>
      </div>
...
export default function MapPage() {
  ...
  const venuesWithDistance = useMemo(() =>
    VENUES.map(v => ({
      ...v,
      distance: location ? getDistance(location.lat, location.lng, v.coordinates.lat, v.coordinates.lng) : undefined,
    })), [location]);

  const sorted = useMemo(() => {
    const list = [...venuesWithDistance];
    if (sortBy === 'distance' && location) list.sort((a, b) => (a.distance ?? 99) - (b.distance ?? 99));
    else if (sortBy === 'vibe') list.sort((a, b) => getVibe(b.id).score - getVibe(a.id).score);
    ...
    <div className="w-full max-w-[800px] mx-auto px-4 py-8 pb-24">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--muted)] max-w-md">
          Find the best spots in Toronto to watch WC 2026. Sorted by vibe — because atmosphere matters as much as the scoreline.
        </p>
      </div>

      {/* ── LEGEND ── */}
      <div className="flex flex-wrap gap-2 mb-6">
        {(Object.entries(RUSH_CONFIG) as [string, typeof RUSH_CONFIG.Chill][]).map(([label, cfg]) => (
          <div
            key={label}
        ...
        <span className="text-[10px] text-[var(--muted)] flex items-center ml-1">crowd level</span>
      </div>

      {/* ── CONTROLS ── */}
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        <button
          onClick={() => { getCurrentPosition(); setSortBy('distance'); }}
          disabled={loading}
        ...
        {(['vibe', 'name', 'distance'] as const).map(s => (
          ...
          >
            {s === 'vibe' ? '⚽ Best Vibe' : s === 'name' ? 'A–Z' : '📍 Nearest'}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3 mb-6 p-3 rounded-xl bg-[var(--card)] border border-[var(--border)]">
        ...
        <span className="text-xs font-medium text-[var(--text)]">{VENUES.length} venues</span>
        <span className="text-[var(--muted)] text-xs">·</span>
        <span className="text-xs text-[var(--muted)]">Toronto, ON</span>
        <span className="ml-auto text-[10px] text-[var(--muted)]">Simulated crowd data</span>
      </div>
