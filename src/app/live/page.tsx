...
const FIXTURES = [
  { opponent: 'TBD (Group Draw Q4 2025)', date: 'Mid-June 2026', venue: 'TBD', status: 'Draw Pending' },
  { opponent: 'TBD (Group Draw Q4 2025)', date: 'Mid-June 2026', venue: 'TBD', status: 'Draw Pending' },
  { opponent: 'TBD (Group Draw Q4 2025)', date: 'Mid-June 2026', venue: 'TBD', status: 'Draw Pending' },
];
    ...
    <div className="w-full max-w-[860px] mx-auto px-4 py-8 pb-24">
      ...
      {mounted && (
        <section className="relative rounded-3xl overflow-hidden mb-10 border border-[var(--border)]">
          ...
          <div className="relative z-10 px-6 py-10 text-center">
            <p className="text-xs font-bold uppercase tracking-[0.25em] text-[var(--muted)] mb-2">Kickoff in</p>
            <div className="grid grid-cols-4 gap-3 max-w-sm mx-auto mb-4">
              {[
                ...
                <div key={i} className="flex flex-col items-center bg-[var(--bg)]/60 backdrop-blur-sm border border-[var(--border)] rounded-2xl py-5">
                  <span className={`font-display text-4xl md:text-5xl font-bold tabular-nums leading-none ${i === 3 ? 'text-[var(--accent)]' : 'text-[var(--text)]'}`}>
                    {seg.v}
                  </span>
                  <span className="text-[10px] text-[var(--muted)] uppercase tracking-widest mt-2">{seg.l}</span>
                </div>
              ))}
            </div>
            <p className="text-sm text-[var(--muted)]">
              ⚽ <strong className="text-[var(--text)]">June 11, 2026</strong> · Estadio Azteca · Mexico City
            </p>
          </div>
        </section>
      )}
      ...
      <section className="mb-10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="font-display text-2xl">{displayTeam.name}'s <span className="text-[var(--accent-2)]">Journey</span></h2>
        </div>
        <div className={`rounded-2xl bg-gradient-to-br border border-[var(--border)] overflow-hidden ${
          isCanada ? 'from-[#cc0000]/10 via-[var(--card)] to-[var(--bg-2)]' : 'from-[var(--accent)]/10 via-[var(--card)] to-[var(--bg-2)]'
        }`}>
          <div className="px-5 py-3 border-b border-[var(--border)] flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-widest text-[var(--muted)]">Group Stage Fixtures</span>
          ...
          {FIXTURES.map((f, i) => (
            <div key={i} className="flex items-center gap-4 px-5 py-4 border-b border-[var(--border)]/50 last:border-b-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold text-[var(--text)] mb-0.5">Match {i + 1} vs {f.opponent.split(' (')[0]}</div>
                <div className="text-[10px] text-[var(--muted)]">{f.date} · {f.venue}</div>
              </div>
          ...
          <div className="px-5 py-3 text-[11px] text-[var(--muted)] bg-[var(--bg-2)]/50">
            🎯 {displayTeam.name} {isCanada ? 'qualified for their second ever World Cup' : 'enters the world stage as a heavy-weight contender'}. Group draw expected Q4 2025.
          </div>
        </div>
      </section>
        ...
        <div className="flex gap-3 overflow-x-auto pb-3 no-scrollbar">
          {WC_PHASES.map((phase, i) => (
            ...
            >
              ...
              <h3 className="font-display text-sm text-[var(--text)] leading-tight mb-1">{phase.label}</h3>
              <p className="text-[11px] font-bold text-[var(--accent)] mb-1">{phase.dates}</p>
              <p className="text-[10px] text-[var(--muted)] leading-snug">{phase.note}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <div className="flex flex-col gap-3">
          {[
            { icon: '🌐', title: 'Group Stage Draw', date: 'Q4 2025 (TBC)', note: 'Find out all 12 groups' },
            { icon: '🏟️', title: 'Opening Match', date: 'June 11, 2026', note: 'Estadio Azteca — the iconic start' },
            { icon: displayTeam.flag, title: `${displayTeam.name} First Match`, date: 'Mid-June 2026', note: 'Group stage — exact date TBD post-draw' },
            { icon: '👑', title: 'World Cup Final', date: 'July 19, 2026', note: 'MetLife Stadium, New Jersey' },
          ].map((d, i) => (
            <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-[var(--card)] border border-[var(--border)] hover:border-[var(--border-hover)] transition-all">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-[var(--text)] mb-0.5">{d.title}</div>
        ...
        >
          <div>
            <div className="text-sm font-semibold text-[var(--text)]">Official FIFA Info & Tickets</div>
            <div className="text-xs text-[var(--muted)]">Visit FIFA.com for registrations</div>
          </div>
          <span className="text-[var(--muted)] group-hover:text-[var(--accent)] group-hover:translate-x-1 transition-all text-lg">→</span>
        </a>
      </section>
    </div>
  );
